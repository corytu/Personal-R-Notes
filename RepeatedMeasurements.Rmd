...
In the following paragraphs, I will build two models based on the pseudo-data. One is simple linear regression model, and the other is mixed effect model. I will use `lm` and `lmer` function in R respectively.
...
To address this problem, let's instead try mixed effect model, where "user" will also be considered as one feature of random effect.
...
* [Mixed model](https://en.wikipedia.org/wiki/Mixed_model) on Wikipedia